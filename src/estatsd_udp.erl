...
init([]) ->
  ...
  io:format("Ronny:init called~n",[]),
  MaxBatchSize = 50,
  MaxBatchAge = 1000,
  io:format("Ronny:init called 1 ~w ~w ~n",[MaxBatchSize,MaxBatchAge]),
  Port = 8125, % 3344,
  io:format("Ronny:init called 2 ~w ~n",[Port]),
  ServerOpts = [binary, {active, once}],
  % Open the socket
  io:format("Ronny:init called 3 ~w~n",[ServerOpts]),
  {ok, Socket} = gen_udp:open(Port, ServerOpts),
  io:format("Ronny:init called 4",[]),
